...
ternary code is ahead of the thermometer code by 0.061 on average, but
at matched cell count (thermometer at 768~b) the two are within 0.009
on the cross-domain mean, and a frozen projection on a multi-bit
Euclidean matchline buys no accuracy over Hamming distance, so on this
grid the cheaper cell is sufficient.\ib{Items 1 and 8(a).} A two-stage shortlist recovers most of the
...
specific: Eva-CAM characterization at a stated node to turn the array
estimates into measurements, now that the GPU side is measured
...
\end{pnote}
\ib{Sep 17 status on this list: (2) done (Sec.~V-B); (3) half done --
the GPU baseline is measured, the Eva-CAM runs are not; (1), (4), (5)
open.}
